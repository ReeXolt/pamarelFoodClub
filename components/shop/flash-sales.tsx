...
import type { Product } from '../allProducts/data';
...
export function FlashSales() {
	...
	const fetchFlashSales = async (): Promise<void> => {
		try {
			const response = await fetch('/api/products/flash-sales');

			const data: FlashSalesResponse = await response.json();
	...
	const formatTimeUnit = (value?: number): string | number => {
			...
			<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
					{products.map((product) => {
						...
						return (
							<div key={String(product._id)} className="group">
											...
											<div className="absolute bottom-0 left-0 right-0 bg-gray-200 h-1">
												<div
													...
													style={{
														width: `${Math.min(
															(product.numberSold /
																(product.stock + product.numberSold)) *
																100,
															100,
														)}%`,
													}}
